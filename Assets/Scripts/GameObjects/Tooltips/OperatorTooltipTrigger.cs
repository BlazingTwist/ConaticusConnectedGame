...
namespace GameObjects.Tooltips {
	...
	public class OperatorTooltipTrigger : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
		...
		public void OnPointerExit(PointerEventData eventData) {
			...
			OperatorTooltip.Instance.gameObject.SetActive(false);
		}

		private IEnumerator ShowTooltipLater(float time) {
			yield return new WaitForSecondsRealtime(time);

			OperatorTooltip operatorTooltip = OperatorTooltip.Instance;
			operatorTooltip.PrepareDisplay(OperatorType);
			operatorTooltip.gameObject.SetActive(true);
		}
